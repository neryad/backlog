...
const PLATFORM_META: {
...
}[] = [
  {
    ...
    label: "PlayStation",
    badge: "PS",
    color: colors.platformPSN,
  },
  {
    key: "xbox_gamertag",
    label: "Xbox",
    badge: "XB",
    color: colors.platformXbox,
  },
  {
    key: "switch_code",
    label: "Switch",
    badge: "NSW",
    color: colors.platformSwitch,
  },
  {
    key: "steam_id",
    label: "Steam",
    badge: "STM",
    color: colors.platformSteam,
  },
...
export default function ProfileScreen() {
  ...
  const handleShare = useCallback(async () => {
    ...
    if (profile.switch_code)
      lines.push(`Nintendo Switch: ${profile.switch_code}`);
  ...
  async function loadProfile() {
    ...
    try {
      const { data: profileData } = await supabase
        .from("profiles")
        .select(
          "id, username, display_name, avatar_url, psn_id, xbox_gamertag, switch_code, steam_id, epic_id",
        )
                ...
                <View style={styles.platformsRow}>
                  {PLATFORM_META.filter((p) => profile?.[p.key]).map((p) => (
                    <View key={p.key} style={styles.platformChip}>
                      ...
                      >
                        <Text
                          style={[styles.platformBadgeText, { color: p.color }]}
                        >
                          {p.badge}
                        </Text>
                      </View>
              ...
              <View style={styles.entryMeta}>
                <View
                  style={[
                    ...
                    {
                      backgroundColor:
                        (STATUS_COLORS[item.status] ?? colors.foregroundMuted) +
                        "33",
                    },
                  ]}
                >
